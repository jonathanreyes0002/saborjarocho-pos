...
async function getPinKeyMaterial() {
  ...
  const rawKey = new TextEncoder().encode(pinHash.slice(0, 32));
...
async function encryptToken(tokenData) {
  ...
  return JSON.stringify({ iv: bufferToBase64(iv.buffer), data: bufferToBase64(ciphertext) });
}
  ...
  return new Promise((resolve, reject) => {
    tokenClient = google.accounts.oauth2.initTokenClient({
      ...
      callback:  async (tokenResponse) => {
        ...
        const tokenData = { access_token: tokenResponse.access_token, expiry_time: Date.now() + 3600000 };
        try {
        ...
        if (!tokenClient) {
          tokenClient = google.accounts.oauth2.initTokenClient({
            ...
            callback:  async (tokenResponse) => {
              ...
              const newData   = { access_token: tokenResponse.access_token, expiry_time: Date.now() + 3600000 };
...
function buildEmailBody(toEmail, orderData, detalles, config) {
  ...
  const itemRows = detalles.map((d, i) => {
    ...
    const porcionLabel = d.porcion === 'unidad' ? '1 unidad'
      : d.porcion === 'media' ? '½ orden' : '1 orden';
    const notasRow = d.notas
      ? `<tr style="background:${bg}"><td colspan="5" style="padding:2px 10px 7px;color:#888;font-style:italic;font-size:13px;">${d.notas}</td></tr>`
      : '';
    return `
      <tr style="background:${bg}">
        <td style="padding:7px 10px;">${d.articulo}</td>
        <td style="padding:7px 10px;text-align:center;">${porcionLabel}</td>
  ...
  // Discount rows
  const subtotalOriginal = (orderData.subtotal || 0) + (orderData.descuento_monto || 0);
  const discountRows = orderData.descuento_pct > 0 ? `
      <tr>
        <td colspan="4" style="padding:6px 10px;text-align:right;color:#555;font-size:13px;">
          Subtotal
        </td>
        <td style="padding:6px 10px;text-align:right;font-size:13px;">${formatMXN(subtotalOriginal)}</td>
      </tr>
      <tr>
        <td colspan="4" style="padding:6px 10px;text-align:right;color:#27500A;font-size:13px;">
          Descuento (${orderData.descuento_pct}%)${orderData.descuento_motivo ? ' — ' + orderData.descuento_motivo : ''}
        </td>
        <td style="padding:6px 10px;text-align:right;color:#27500A;font-size:13px;">-${formatMXN(orderData.descuento_monto)}</td>
      </tr>` : '';
  ...
  const clienteInfo = (orderData.cliente_email || orderData.cliente_telefono) ? `
    <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e0e0e0;border-radius:6px;margin-bottom:18px;">
      <tr style="background:#f4f6f8;">
        <td colspan="2" style="padding:8px 12px;font-weight:700;font-size:13px;color:#555;">DATOS DEL CLIENTE</td>
      </tr>
      ${orderData.cliente_email    ? `<tr><td style="padding:5px 12px;color:#555;font-size:13px;">Email</td><td style="padding:5px 12px;font-size:13px;">${orderData.cliente_email}</td></tr>` : ''}
      ${orderData.cliente_telefono ? `<tr><td style="padding:5px 12px;color:#555;font-size:13px;">Teléfono</td><td style="padding:5px 12px;font-size:13px;">${orderData.cliente_telefono}</td></tr>` : ''}
    </table>` : '';
  ...
  try {
    ...
    if (files.length > 0) {
      ...
      const multipart = [
        ...
        `--${boundary}`, 'Content-Type: text/csv; charset=UTF-8', '',
        combined, `--${boundary}--`
      ].join('\r\n');

      const patchRes = await fetch(
        `https://www.googleapis.com/upload/drive/v3/files/${fileId}?uploadType=multipart`,
        { method: 'PATCH', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': `multipart/related; boundary=${boundary}` }, body: multipart }
      );
      if (!patchRes.ok) { const err = await patchRes.text(); return { success: false, error: err }; }
      ...
      if (!createRes.ok) { const err = await createRes.text(); return { success: false, error: err }; }
...
export function buildBackupCSV(orders, detallesMap) {
  const header = 'Folio,Mesa,Fecha,Hora,Estado,Método Pago,Subtotal,Descuento%,Descuento$,Efectivo Recibido,Cambio,Cliente Email,Artículo,Porción,Cantidad,Precio Unit,Subtotal Línea,Notas\n';
  const rows = [];
  for (const order of orders) {
    const detalles = detallesMap[order.id_orden] || [];
    if (detalles.length === 0) {
      rows.push([
        order.folio || '', order.nombre_mesa, order.fecha, order.hora_orden,
        order.estado, order.metodo_pago || '', order.subtotal || 0,
        order.descuento_pct || 0, order.descuento_monto || 0,
        order.efectivo_recibido || '', order.cambio || '',
        order.cliente_email || '', '', '', '', '', '', ''
      ...
      detalles.forEach(d => {
        rows.push([
          ...
          order.descuento_pct || 0, order.descuento_monto || 0,
          order.efectivo_recibido || '', order.cambio || '',
          order.cliente_email || '', d.articulo, d.porcion, d.cantidad,
          d.precio_unitario, d.subtotal_linea, d.notas || ''
        ].map(v => `"${String(v).replace(/"/g, '""')}"`).join(','));
      });
    }
  }
